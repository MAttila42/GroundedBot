...
public class Setup : InteractionModuleBase
{
	...
	{
		await RespondAsync(embed: Embeds.Info("Üzenet küldése..."), ephemeral: true);
		switch (message)
		{
			case Message.CreateClass:
				...
				catch (Exception e) { await FollowupAsync(embed: Embeds.Error("Nem sikerült elküldeni az üzenetet", $"```{e.Message}```"), ephemeral: true); }
				break;
			default:
				await FollowupAsync(embed: Embeds.Error("Nem sikerült elküldeni az üzenetet", $"Nincs {message} üzenet."), ephemeral: true);
				break;
		}
	}
}
