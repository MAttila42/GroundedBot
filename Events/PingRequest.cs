    ...
    public class PingRequest : MessageEvent
    {
        public async Task Execute()
        {
            ...
            EmbedBuilder requestEmbed = new EmbedBuilder()
                .WithAuthor("Új ping kérelem", EmbedService.PlusIcon)
            ...
            ComponentBuilder components = new ComponentBuilder()
                .WithButton(customId: $"prbutton-approve:{Context.Channel.Id},{pingMsg.Id},{role.Id}", style: ButtonStyle.Success, emote: _emoji.GetEmoji("util_checkmark"))
                .WithButton(customId: $"prbutton-reject:{Context.Channel.Id},{pingMsg.Id}", style: ButtonStyle.Danger, emote: _emoji.GetEmoji("util_cross"));
            ...
            catch (Exception e) { await pingMsg.ModifyAsync(m => m.Embed = EmbedService.Error("Nem sikerült elküldeni a ping kérést", $"Szólj egy adminisztrátornak, hogy nem találok `PingRequest` role-t, `PingRequest` csatornát, vagy nincs jogom üzenetet küldeni oda!\n```{e.Message}```")); }
        }
    }
}
